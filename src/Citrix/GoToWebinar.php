...
class GoToWebinar extends ServiceAbstract implements CitrixApiAware
{
  ...
  public function register($webinarKey, $registrantData){
    ...
    $this->setHttpMethod('POST')
        ...
        ->addHeader('Accept', 'application/vnd.citrix.g2wapi-v1.1+json')
        ->sendRequest($this->getClient()->getAccessToken())
        ->processResponse(true);
  ...
  public function getRegistrationFields($webinarKey) {
    ...
    $url =  'https://api.getgo.com/G2W/rest/organizers/' . $this->getClient()->getOrganizerKey() . '/webinars/' . $webinarKey . '/registrants/fields';
    $this->setHttpMethod('GET')
      ->setUrl($url)
      ->sendRequest($this->getClient()->getAccessToken())
      ->processResponse(true);
